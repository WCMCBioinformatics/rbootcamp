...
```{r}
pa_tidy %>% distinct(Event_num, Date_of_Isolation)
unique(pa_tidy$Event_num) #base
```

`count` and `tally` count occurances
```{r}
pa_tidy %>% count(Date_of_Isolation)
pa_tidy %>% group_by(Date_of_Isolation) %>% tally()
```
...
What are the mean antibiotic scores for each event per antibiotic?
```{r}
pa_tidy %>% 
